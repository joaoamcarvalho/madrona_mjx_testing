...
arg_parser.add_argument('--batch-render-view-width', type=int, default=1280)
arg_parser.add_argument('--batch-render-view-height', type=int, default=720)
arg_parser.add_argument('--add-cam-debug-geo', action='store_true')
arg_parser.add_argument('--use-rasterizer', action='store_true')
arg_parser.add_argument('--show-plot', action='store_true')
...
if __name__ == '__main__':  
  ...
  for i in range(5):
    st = time.time()
    state, rgb, depth = step(state)
    print(f"Step {i}/5 successful, time: {time.time() - st:.3f}s")
